...
const WelcomePopup = ({ data }) => {
  ...
  const handleContinueClicked = (event) => {
    ...
    if (selectedItem) {
      if (selectedItem.type === 'origin') {
        ...
        localStorage.setItem('location', JSON.stringify(selectedItem.name));
      } else {
        dispatch(setLanguage(selectedItem));
        localStorage.setItem('language', JSON.stringify(selectedItem.name));
      }
    }
  };
  ...
  return (
    <form onSubmit={handleContinueClicked}>
